...
const FTROT: Transform = Transform::from_xyz(-1., FLOOR, 0.);
const FTROT_MESHLET: Transform = Transform::from_xyz(1., FLOOR, 0.);
...
fn setup(
    ...
    let ftrot = assets.load("models/foxtrot_fox/ftrot.gltf#Scene0");
    commands.spawn((SceneRoot(ftrot), FTROT));

    let ftrot_meshlet = assets.load("meshlets/ftrot.meshlet");
    commands.spawn((
        MeshletMesh3d(ftrot_meshlet),
        MeshMaterial3d(debug_material.clone()),
        FTROT_MESHLET,
    ));
}
